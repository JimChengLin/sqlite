...
8. 部分完成：optimistic transaction view。当前已有 committed generation、短 reader-view pin、旧 generation key-change retention/prune、pinned-generation point read reconstruction、writer point read set、seek-path result-bounded byte-range read set、commit 冲突校验和 direct adapter tests；stale writer read-set/range 冲突返回 `SQLITE_BUSY_SNAPSHOT`，而不是可等待的 generic busy。还缺 generation-aware range seek/metadata visibility、SQL statement/cursor 生命周期 pin、SQL 层 `SQLITE_BUSY_SNAPSHOT` 传播覆盖，以及把普通读可见性从 transaction-start snapshot 完整迁到 generation view。显式长读事务在这个能力完整前 fail fast 或保持 rollback-journal busy 语义。
