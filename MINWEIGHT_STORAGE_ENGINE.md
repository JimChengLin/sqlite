...
- P0: finish replacing remaining materialized root consumers. Int-key table movement, non-int-key sequential movement, versioned-root `BtreeIndexMoveto`, and root clear/move now use seek/range APIs; remaining `loadRows()` users are fallback cursor restoration, backup/copy-style whole-root flows, and stats/check helpers that still need range/batch implementations.
...
- P1: continue replacing root/store maintenance fallbacks. `clearRoot`/`moveRoot` now use seek iteration for int-key and versioned non-int-key roots; `BtreeCopyFile` and other whole-root copy/check flows still need range delete/root rewrite and batch operations.
