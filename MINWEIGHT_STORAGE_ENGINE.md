...
- Added the first optimistic transaction-view foundation: committed generation numbers, short reader-view pins, retained per-generation committed key changes, writer point-read tracking, root-level range-read tracking, commit conflict validation, and pruning of old generation changes after the last reader releases them. Current coverage is direct adapter-level validation for pinned old-version retention, point read-set conflict rejection, root range-read conflict rejection, and unrelated-root non-conflict; byte-range read sets and full SQL statement lifecycle integration remain P0.
...
- P1: continue replacing root/store maintenance fallbacks. `clearRoot`/`moveRoot` now use seek iteration for int-key and versioned non-int-key roots; `BtreeCopyFile` now uses streaming batch/overlay copy, but still scans source and target because the operation replaces a whole btree. Remaining copy/check flows need narrower range operations where the SQLite operation is root-scoped rather than whole-database scoped.
