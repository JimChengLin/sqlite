...
- Continued splitting the large minweight adapter file by moving root clear/move maintenance into `lib/minweight_storage_engine_roots.go` and cursor movement/positioning into `lib/minweight_storage_engine_cursor.go`; key encoding, cursor lifecycle/payload, transaction view, and pager/file shims are still planned split boundaries.
...
- Wired pinned generation point reads into the adapter `get` path. A reader pinned to an older generation now reconstructs updated, inserted, and deleted keys from retained commit changes for point lookup; range seek and metadata visibility still need generation-view reconstruction.
...
- P0: finish optimistic transaction-view integration. The current foundation tracks committed generations, point read sets, result-bounded seek-path byte-range read sets, reader pins, retained key changes, pinned-generation point reads, and commit conflict tests; next steps are generation-aware range seek/metadata visibility, SQL statement/cursor pin boundaries, SQL-level `SQLITE_BUSY_SNAPSHOT` propagation coverage for stale writer conflicts, and removal of transaction-start snapshot reads from normal visibility.
...
- P1: split `lib/minweight_storage_engine.go` into focused files, with likely boundaries for handles/lifecycle, metadata persistence, transaction overlay/generation view, key encoding, cursor lifecycle/payload, backup/serialize, and pager/file compatibility shims.
