...
- P0: finish replacing remaining materialized root consumers. Int-key table movement, non-int-key sequential movement, versioned-root `BtreeIndexMoveto`, root clear/move, `BtreeCopyFile`, `BtreeCursorRestore`, stale materialized versioned index `Next`/`Previous`/`Eof`, and int-key stats recompute now avoid `loadRows()`; the remaining production caller is `refreshCursorRows()` for cursors that lack a usable versioned store-key anchor.
