...
- Wired pinned generation point reads into the adapter `get` path. A reader pinned to an older generation reconstructs updated, inserted, and deleted keys from retained commit changes for point lookup; metadata visibility still needs generation-view reconstruction.
...
- P0: finish optimistic transaction-view integration. The current foundation tracks committed generations, point read sets, result-bounded seek-path byte-range read sets, reader pins, retained key changes, pinned-generation point/range reads, and commit conflict tests; next steps are generation-aware metadata visibility, SQL statement/cursor pin boundaries, SQL-level `SQLITE_BUSY_SNAPSHOT` propagation coverage for stale writer conflicts, and removal of transaction-start snapshot reads from normal visibility.
