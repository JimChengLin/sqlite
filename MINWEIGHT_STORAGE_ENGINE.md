...
- Moved `BtreeCursorRestore` for stale versioned index cursors onto point lookup plus `SeekGE`, moved stale/materialized versioned index `Next`/`Previous`/`Eof` onto seek paths, and moved int-key stats recompute onto seek iteration.
- Removed `loadRows()` / `refreshCursorRows()` from the minweight adapter. Versioned index cursors must carry a versioned store-key anchor; missing anchors now fail fast with `SQLITE_CORRUPT` instead of scanning and materializing the root.
- Split cursor-restore tests into `lib/minweight_storage_engine_cursor_test.go` and added direct coverage for stale index cursor restore, stale materialized index `Next`/`Previous`, stale old-index EOF/next seeking past deleted rows, and missing store-key fail-fast behavior.
...
- Added the first optimistic transaction-view foundation: committed generation numbers, short reader-view pins, retained per-generation committed key changes, writer point-read tracking, byte-range read tracking for seek paths, commit conflict validation, and pruning of old generation changes after the last reader releases them. Current coverage is direct adapter-level validation for pinned old-version retention, point read-set conflict rejection, byte-range conflict rejection, and same-root range-outside non-conflict; full SQL statement lifecycle integration remains P0.
...
- Full historical `golangci-lint run ./lib --enable-only=gocyclo,funlen --timeout 5m`: useful when specifically splitting old debt, but not a routine gate yet. Current new-code gate is `--new`; known old debt includes large minweight btree entrypoints such as cursor movement, insert/delete, and historical test helpers.
...
- P0: finish replacing remaining materialized root consumers. Int-key table movement, non-int-key sequential movement, versioned-root `BtreeIndexMoveto`, root clear/move, `BtreeCopyFile`, `BtreeCursorRestore`, stale materialized versioned index `Next`/`Previous`/`Eof`, and int-key stats recompute now avoid root materialization. `loadRows()` / `refreshCursorRows()` have been removed from the adapter; any future migration/import path for old raw index keys must be explicit and isolated from normal cursor movement.
...
- P0: finish optimistic transaction-view integration. The current foundation tracks committed generations, point read sets, seek-path byte-range read sets, reader pins, retained key changes, and commit conflict tests; next steps are tighter result-bounded ranges where current seek ranges are still conservative, SQL statement/cursor pin boundaries, busy-handler behavior for conflicts, and removal of transaction-start snapshot reads from normal visibility.
